...
"Varietas" 是拉丁语中的一个单数形式，复数形式为 "varietates"，意为 "变种"、"品种" 或 "变异"。在生物分类学中，它通常用来描述同种不同亚型或基因型的变异，并可以作为生物分类的层级之一。

"Forma" 是拉丁语中的一个单数形式，复数形式为 "formae"，意为 "形态" 或 "型"。在生物学中，它通常用于描述同种不同外形或形态的变化，例如同一物种的不同亚种、变种、变型等。

界（Kingdom） 门（Phylum） 纲（Class） 目（Order） 科（Family） 属（Genus） 种（Species）

# 生物学术语

#1. Biosample（生物样品）：它是指从一个生物体中采集的组织、细胞或其他生物学材料的样品。Biosample 可以是来自植物、动物、微生物等各种生物体的样品，用于进行基因组分析、表达研究、遗传变异等实验。
...
# newick-utils
NNewick-Utils 是一个用于处理和分析 Newick 格式的工具集合。Newick 格式是一种常用的表示进化树或系统发育树的文本格式。Newick-Utils 提供了一系列命令行工具，可以对 Newick 格式进行解析、显示、修改和分析。
...
是用于定位和访问互联网资源的地址。它是在 Web 上标识和定位资源的标准方式。

URL 通常由以下几个部分组成：

1. **协议（Protocol）**：协议指定了访问资源所使用的规则和方法，如 HTTP、HTTPS、FTP 等。

2. **主机（Host）**：主机指定了存储资源的服务器的域名或 IP 地址。

3. **端口（Port）**：端口指定了与资源交互的进程或服务的编号，如 HTTP 的默认端口是 80，HTTPS 的默认端口是 443。

4. **路径（Path）**：路径指定了资源在服务器上的位置，可以是一个文件路径或目录路径。

5. **查询字符串（Query String）**：查询字符串用于向服务器传递额外的参数，以便获取特定的数据或执行特定的操作。它通常以 `?` 开头，键值对以 `&` 连接。

6. **片段标识（Fragment Identifier）**：片段标识指定了资源中的一个特定片段或锚点。通常以 `#` 开头。
...
这个 URL 使用 HTTPS 协议访问主机为 `www.example.com` 的服务器，端口为 8080，路径为 `/path/to/resource`，包含查询参数 `param1=value1` 和 `param2=value2`，还有一个片段标识为 `section1`。

需要注意的是，URL 中的各个部分并非都是必需的。

# datamash check 
是一个命令行工具 datamash 的一部分，用于检查输入数据的一致性和完整性。

datamash 是一个强大的文本处理工具，它可以用于对文本文件中的数据进行聚合、统计和转换操作。其中的 check 子命令可以用来检查输入文件的一些常见问题，如缺失值、重复行和列不匹配等。

# FTP（文件传输协议）和HTTP（超文本传输协议）
都是用于在网络上传输数据的协议，但它们在设计和用途上有一些区别。

1. FTP（File Transfer Protocol  文件传输协议）：
   - 用途：FTP 主要用于文件的上传、下载和管理。它提供了一个专门的服务器来存储和共享文件，并允许用户通过客户端应用程序与服务器进行连接和交互。
   - 工作方式：FTP 使用控制连接和数据连接来传输文件。控制连接用于发送命令和获取服务器响应，而数据连接则用于实际的文件传输。
   - 安全性：FTP 不提供加密功能，数据在传输过程中是以明文形式发送的。

2. HTTP（The Hypertext Transfer Protocol 超文本传输协议）：
   - 用途：HTTP 主要用于在客户端和服务器之间传输和浏览超文本文档，也就是我们通常浏览器中打开的网页内容。它支持各种不同的网络资源，如 HTML 文档、图像、音频、视频等。
   - 工作方式：HTTP 是一个无状态协议，每次请求都是独立的。客户端向服务器发送 HTTP 请求，并且服务器以 HTTP 响应的形式返回请求的资源。
   - 安全性：HTTP 不是加密的协议，数据在传输过程中以明文形式发送。然而，可以通过 HTTPS（HTTP 安全）协议来加密 HTTP 连接，并确保数据的加密和身份验证。

总结来说，FTP 主要用于文件传输和管理，而 HTTP 主要用于在客户端和服务器之间传输超文本资源。FTP 提供了对文件的上传和下载的支持，而 HTTP 更适用于传输网页内容和其他网络资源。此外，FTP 不提供加密功能，而 HTTPS 则是使用 HTTP 协议的安全版本，可提供数据的加密和身份验证。

# SSH（Secure Shell）
是一种网络协议，用于在不安全的网络中安全地远程登录和执行命令。它为远程计算机之间的通信提供了加密和身份验证的功能。

SSH 的主要特点包括：

1. 安全性：SSH 使用加密技术，包括对称加密、公钥加密和消息认证码等，以确保传输的数据在网络中是安全的。这样，即使在不可信的网络上进行通信，也可以防止数据被窃听或篡改。

2. 远程登录：SSH 允许用户通过 SSH 客户端连接到远程计算机，并登录到远程系统的命令行界面，就像在本地计算机上一样操作远程计算机。

3. 文件传输：SSH 还支持安全的文件传输功能。用户可以使用 SCP（Secure Copy Protocol）或 SFTP（Secure File Transfer Protocol）命令来在本地计算机和远程计算机之间传输文件，确保文件的机密性和完整性。

4. 端口转发：SSH 支持端口转发功能，允许将本地计算机上的网络服务通过 SSH 连接安全地映射到远程计算机上。

5. 公钥身份验证：SSH 支持公钥身份验证机制，允许用户使用密钥对进行身份验证，而无需每次输入密码。这提供了更高的安全性和方便性。

SSH 在系统管理、远程维护、文件传输和安全通信等方面都得到广泛应用。它是一种可靠和安全的远程访问方式，被广泛用于服务器管理、远程登录和文件传输等场景。
